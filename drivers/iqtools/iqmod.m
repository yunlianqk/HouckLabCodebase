...
if (numSamples >= threshold)
    ...
    try
        ...
        ovsPulseShape_I = overlapsave(filt.Numerator');
        ovsPulseShape_Q = overlapsave(filt.Numerator');
        ...
        while (sampleCnt < numSamples)
            ...
            if (cy ~= 0 && ~isempty(wfm))
                shiftSig = exp(1j * 2 * pi * cy * ((sampleCnt:sampleCnt+length(wfm)-1)'/numSamples + phi));
                wfm = wfm .* shiftSig;
            end
            ...
            maxVal = max(max(abs(real(wfm2))),max(abs(real(wfm2))));
            ...
            if (len > 0)
                ...
                switch (fct)
                    ...
                    case 'save'
                        if (sampleCnt == 0)
                            dlmwrite(savefile, [real(wfm) imag(wfm)]);
                        else
                            dlmwrite(savefile, [real(wfm) imag(wfm)], '-append');
                        end
                    case 'none'
                    otherwise 
