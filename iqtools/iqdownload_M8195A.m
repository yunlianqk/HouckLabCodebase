...
if (~isempty(sequence))
    ...
    for ch = find(channelMapping(:,1))'
        gen_arb_M8195A(arbConfig, f, ch, real(data), marker1, segmNum, run, fs, segmentLength, segmentOffset);
    end
    for ch = find(channelMapping(:,2))'
        gen_arb_M8195A(arbConfig, f, ch, imag(data), marker2, segmNum, run, fs, segmentLength, segmentOffset);
    end
