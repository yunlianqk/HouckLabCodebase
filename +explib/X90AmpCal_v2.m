classdef X90AmpCal_v2 < handle
    ...
    methods
        function obj=X90AmpCal_v2(pulseCal,varargin)
        ...
        function [result] = directRunM8195A(obj,awg,card,cardparams,playlist)
            ...
            for ind=1:softavg
                ...
                % normalize amplitude
                xaxisNorm=obj.numGateVector/2; % For fit function pass in # of pairs, equivalent to the pi pulse fitting routine
                amp=sqrt(Pint);
                norm0=amp(end-1);
                norm1=amp(end);
                ...
                if ~mod(ind,10)
                    ...
                    ylabel('Normalized Amplitude'); xlabel('Pairs of Pi/2 Pulses');
                    subplot(2,3,4);
