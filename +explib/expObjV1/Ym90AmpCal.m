classdef Ym90AmpCal< handle
    % minus Pi/2 pulse amplitude error amplification. After an initial pi/2 pulse it will
    % vary the number of subsequent pi pulses.  
    
    properties
        % change these to tweak the experiment
        experimentName = 'Ym90AmpCal';
        numGateVector = 2:2:40;
        qubitFreq=4.772869998748302e9;
        qubitAmplitude = .3575;
        qubitDragAmplitude = .016;
        qubitSigma = 25e-9;
        iGateType = 'Ym90'; % initial gate (currently just use a half power pi pulse... need to rethink how this should be done after calibrations!)
        gateType = 'Ym90'; % repeated gate type 
