classdef TWPASaturationTest < handle
    % alternate between trials in the ground state and trials in the
    % excited. Keep single shot data and histogram them.
    % long pulse beforehand to test if there is a saturation effect 
    
    properties 
        experimentName = 'TWPASaturationTest';
        % inputs
%         rabiFreq = 2e9;
        rabiFreq = 4.772869998748302e9;
        rabiAmp = .05;
        rabiDuration = 50e-6;
        rabiDelay = 1e-6; % delay after rabi pulse before qubit pulse
    ...
    methods
        function obj=TWPASaturationTest(pulseCal,varargin)
...
%             obj.qubit = obj.pulseCal.Identity();
            ...
            obj.rabi = pulselib.measPulse(obj.rabiDuration, obj.rabiAmp);
%             obj.qubitPulseTime = obj.pulseCal.startBuffer+obj.qubit.totalDuration/2;
            % add extra time to fit in the rabi pulse
            obj.qubitPulseTime = obj.pulseCal.startBuffer+obj.qubit.totalDuration/2 + obj.rabiDelay + obj.rabiDuration;
        ...
        function playlist = directDownloadM8195A(obj,awg)
            ...
            % add rabi pulse
            rabiStop = obj.qubitPulseTime - obj.qubit.totalDuration/2 - obj.rabiDelay;
            rabiStart = rabiStop - obj.rabi.totalDuration;
            [iRabi qRabi] = obj.rabi.uwWaveforms(t, rabiStart);
            ialtqc = cos(2*pi*obj.rabiFreq*t);
            qaltqc= sin(2*pi*obj.rabiFreq*t);
            ...
            backgroundWaveform = iMeasMod+qMeasMod+iRabiMod+qRabiMod;
