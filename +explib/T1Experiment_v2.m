classdef T1Experiment_v2 < handle
    ...
    methods
        ...
        function obj=initSequences(obj)
            ...
            sequences(1,length(obj.delayList)+2) = pulselib.gateSequence(); % initialize empty array of gateSequence objects
            for ind = 1:length(obj.delayList)
                delayGateTime = obj.delayList(ind) - obj.qubit.totalDuration/2; % so that pulse delays match the delayList
                delayGate = obj.pulseCal.Delay(delayGateTime);
                gateArray = [obj.qubit delayGate];
