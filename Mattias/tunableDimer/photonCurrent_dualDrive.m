...
for scanType = 1:3
    ...
    drive.powerVec = linspace(-50,5,64);
    % drive.freqVec = linspace(5.818e9,5.89e9,5);
    drive.freqVec = linspace(5.8e9,5.95e9,75);
    ...
    for idx = 1:length(drive.freqVec)
        ...
        for jdx=1:length(drive.powerVec)
            ...
            if jdx==1 && idx==1
                ...
                if scanType == 1 % left
                    filename=['pC_leftInput_fDetunedLargeJ_' num2str(time(1)) num2str(time(2)) num2str(time(3))...
                        num2str(time(4)) num2str(time(5))];
                elseif scanType == 2 % right
                    filename=['pC_rightInput_fDetunedLargeJ_' num2str(time(1)) num2str(time(2)) num2str(time(3))...
                        num2str(time(4)) num2str(time(5))];
                elseif scanType == 3 % both
                    filename=['pC_dualInput_fDetunedLargeJ_' num2str(time(1)) num2str(time(2)) num2str(time(3))...
                        num2str(time(4)) num2str(time(5))];
                end
            end
            ...
            if jdx==1 && idx==1
                deltaT=toc(tStart);
                estimatedTime=deltaT*length(drive.powerVec)*length(drive.freqVec)*3;
    ...
    saveFolder = 'C:\Users\Cheesesteak\Documents\Mattias\tunableDimer\pulses_overnight_072517\';
