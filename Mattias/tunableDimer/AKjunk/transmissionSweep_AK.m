...
%         fsteps=100;
    ...
    %print trajectory paramters
    RQ_min = vtraj(1,1);
    RQ_max = vtraj(1,fsteps);
    
    LQ_min = vtraj(2,1);
    LQ_max = vtraj(2,fsteps);
    
    C_min = vtraj(3,1);
    C_max = vtraj(3,fsteps);
    
%     if sdx ==1
%         disp('Left Qubit Scan')
%     elseif sdx == 2
%         disp('Right Qubit Scan')
%     elseif sdx == 3
%         disp('Coupler Scan')
%     else
%         disp('WTF?!')
%     end
%     disp(['Left Qubit: Vmin = ' num2str(RQ_min) ', Vmax = ' num2str(RQ_max)])
%     disp(['Right Qubit: Vmin = ' num2str(LQ_min) ', Vmax = ' num2str(LQ_max)])
%     disp(['Coupler: Vmin = ' num2str(C_min) ', Vmax = ' num2str(C_max)])
    ...
    for index=1:steps
        if index==1
            ...
            time=clock;
            if sdx == 1
                filename=['AK_couplerFit_LeftInput_leftQubitScan_'  num2str(time(1)) num2str(time(2)) num2str(time(3)) num2str(time(4)) num2str(time(5)) num2str(time(6))];
            elseif sdx == 2
                filename=['AK_couplerFit_LeftInput_rightQubitScan_'  num2str(time(1)) num2str(time(2)) num2str(time(3)) num2str(time(4)) num2str(time(5)) num2str(time(6))];
            else
                filename=['AK_couplerFit_LeftInput_couplerScan_'  num2str(time(1)) num2str(time(2)) num2str(time(3)) num2str(time(4)) num2str(time(5)) num2str(time(6))];
            end
        end
        % update flux/voltage
