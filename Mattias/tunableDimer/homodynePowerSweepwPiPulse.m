...
qubitDrivePowerVec = [-100 linspace(-20,19,20)];
...
%         measurementPoint.powerSetPoints = [linspace(3, 8, 5)];
        ...
        if qubitDrivePowerVec(ldx)<-20
            measurementPoint.name = ['pulseCalibration_qubitToneAtCavity_maxJ_DDDOswitchingCompare_qubitDriveOff_driveAt' num2str(measurementPoint.probeFrequency/1e9)];
        else
            measurementPoint.name = ['pulseCalibration_qubitToneAtCavity_maxJ_DDDOswitchingCompare_qubitDrivePower' num2str(qubitDrivePowerVec(ldx)) 'dBm_driveAt' num2str(measurementPoint.probeFrequency/1e9)];
        end
...
for acq = 1:numAcquisitions
    ...
    mxg.freq = config.probeFrequency;
    logen.freq = mxg.freq;
    ...
    for pdx = 1:length(drive.powerSetPoints)
        ...
        if pdx > 1 && drive.powerSetPoints(pdx) == drive.powerSetPoints(pdx-1)
            ...
            mxg.power = drive.powerSetPoints(pdx);
        end
