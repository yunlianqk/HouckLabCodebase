...
%%
% drive.powerVec = linspace(-30,-20,40);


driveEpsilon = 0.0001;


drive.powerSetPoints = linspace(-30,-24,30);
cardAcqLengths = [5e-6 10e-6 15e-6];

for acq = 1:length(cardAcqLengths)
triggen.period =cardAcqLengths(acq)+2e-6;
...
drive.powerVec = linspace(drive.powerSetPoints(pdx),drive.powerSetPoints(pdx)+driveEpsilon,200);
% drive.powerVec = linspace(-29,-24,100);
...
ampDataAvgPostAvg = zeros(1,length(drive.powerVec));
...
for idx = 1:length(drive.powerVec)
    ...
    ampDataAvgPostAvg(idx) = mean(IDataTemp.^2 + QDataTemp.^2);
...
%     IDataSingle(idx,:) = IDataTemp(1:int32(cardparams.samples))-IDataAvg(idx);
%     QDataSingle(idx,:) = IDataTemp(1:int32(cardparams.samples))-QDataAvg(idx);
...
figure(22);
subplot(2,1,1);
plot(drive.powerVec, ampDataAvg);
title('ampDataAvg');
xlabel('Drive Power [dBm]');

subplot(2,1,2);
plot(drive.powerVec, ampDataAvgPostAvg);
