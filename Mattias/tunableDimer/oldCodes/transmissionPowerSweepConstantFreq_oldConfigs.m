...
% fstop=[leftQubitMin (rightQubitMin+0.15) 0.0];fsteps=20;
% vstart=fc.calculateVoltagePoint(fstart);vstop=fc.calculateVoltagePoint(fstop);
% vtraj=fc.generateTrajectory(vstart,vstop,fsteps);
% % for ldx = 4
% for ldx = [1,2,3,4,5,6]
%     tempdx = tempdx+1;
%     mp = {};
%     mp.name = ['maxJ_RDDO_upperPeak_powerSweep' '_vtraj' num2str(ldx)]; 
...
% %     mp.driveFreqs = linspace(5.91e9,5.915e9,75);
% %     mp.driveFreqs = linspace(5.9e9,5.93e9,100);
%     mp.driveFreqs = linspace(5.9e9,5.95e9,250);
...
% tempdx = tempdx+1;
% mp = {};
% mp.name = ['maxJ_LDDO_lowerPeak_powerSweep_detuned']; 
% mp.voltagePoint = vtraj(:,1);
% mp.startPower = -30;
% mp.stopPower = 10;
% mp.powerNumPoints = 1001;
% mp.drivePowers = linspace(mp.startPower,mp.stopPower,mp.powerNumPoints);
% % mp.driveFreqs = linspace(5.825e9,5.85e9,200);
% mp.driveFreqs = linspace(5.84e9,5.845e9,75);
% mp.freqNumPoints = length(mp.driveFreqs);
% mp.waitTime = 10;
% acquisitionPoints(tempdx) = mp;
% 
...
% tempdx = tempdx+1;
% mp = {};
% mp.name = ['maxJ_LDDO_upperPeak_powerSweep_detuned']; 
% mp.voltagePoint = vtraj(:,1);
% mp.startPower = -30;
% mp.stopPower = 10;
% mp.powerNumPoints = 1001;
% mp.drivePowers = linspace(mp.startPower,mp.stopPower,mp.powerNumPoints);
% % mp.driveFreqs = linspace(5.88e9,5.925e9,200);
% mp.driveFreqs = linspace(5.91e9,5.915e9,75);
% mp.freqNumPoints = length(mp.driveFreqs);
% mp.waitTime = 10;
% acquisitionPoints(tempdx) = mp;
