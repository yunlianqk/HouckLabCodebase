function [ ] = AWG_PulseTuningPulseGenerator4( AWGHandle, pulseamp, pulsequad, dragamp, num_gate)

%pulseamp is [X90p X180p X180m] 
...
if pulsequad == 0
    ...
    waveform1 = [zeros(1,t_wait-(2*num_gate+1)*length(gausspulse)-2*num_gate*t_secpulsewait), pulseamp(1)*amp*gausspulse];
    waveform3 = [zeros(1,t_wait-(2*num_gate+1)*length(gausspulse)-2*num_gate*t_secpulsewait), pulseamp(1)*amp*dragpulse];
    if num_gate ~= 0
        for counter1=1:num_gate
            waveform1 = [waveform1, zeros(1,t_secpulsewait), pulseamp(2)*amp*gausspulse, zeros(1,t_secpulsewait), pulseamp(3)*amp*gausspulse];
            waveform3 = [waveform3, zeros(1,t_secpulsewait), pulseamp(2)*amp*dragpulse, zeros(1,t_secpulsewait), pulseamp(3)*amp*dragpulse];
        end
    end
else
    waveform1 = [zeros(1,t_wait-(2*num_gate+1)*length(gausspulse)-2*num_gate*t_secpulsewait), pulseamp(1)*amp*dragpulse];
    waveform3 = [zeros(1,t_wait-(2*num_gate+1)*length(gausspulse)-2*num_gate*t_secpulsewait), pulseamp(1)*amp*gausspulse];
    % rotation should be around y axis
    if num_gate ~= 0
        for counter1=1:num_gate
            waveform1 = [waveform1, zeros(1,t_secpulsewait), pulseamp(2)*amp*dragpulse, zeros(1,t_secpulsewait), pulseamp(3)*amp*dragpulse];
            waveform3 = [waveform3, zeros(1,t_secpulsewait), pulseamp(2)*amp*gausspulse, zeros(1,t_secpulsewait), pulseamp(3)*amp*gausspulse];
        end
    end
end
