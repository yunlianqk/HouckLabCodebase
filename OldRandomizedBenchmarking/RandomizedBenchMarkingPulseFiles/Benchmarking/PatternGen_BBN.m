...
classdef PatternGen < handle
    ...
    methods (Static)
        ...
        function [outx, outy] = gaussSquarePulse(amp, n, sigma, varargin)
            numSigmas = 6;
            % currently hard coded to 4 sigma gaussians
            turnon = gaussOnPulse(amp, numSigmas*sigma/2, sigma);
            middle = amp*ones(n-numSigmas*sigma, 1);
            turnoff = gaussOffPulse(amp, numSigmas*sigma/2, sigma);
        ...
        function [outx, outy] = derivGaussOnPulse(amp, n, sigma, varargin)
            t = 1:n;
            outx = round(amp * (-(t-n)./sigma^2).*exp(-(t - n).^2./(2 * sigma^2))).';
            outy = zeros(n, 1);
        end
        
        function [outx, outy] = derivGaussOffPulse(amp, n, sigma, varargin)
            t = 1:n;
            outx = round(amp *(-(t-1)./sigma^2).* exp(-(t-1).^2./(2 * sigma^2))).';
            outy = zeros(n, 1);
        end
        
        function [outx, outy] = derivGaussSquarePulse(amp, n, sigma, varargin)
            numSigmas = 6;
            % currently hard coded to 4 sigma gaussians
            turnon = derivGaussOnPulse(amp, numSigmas*sigma/2, sigma);
            middle = amp*zeros(n-numSigmas*sigma, 1);
            turnoff = derivGaussOffPulse(amp, numSigmas*sigma/2, sigma);
            outx = [turnon; middle; turnoff];
            outy = zeros(n, 1);
        end
  
        function [outx, outy] = dragSqPulse(amp, n, sigma, delta)
            self = PatternGen;
            [outx, tmp] = self.gaussSquarePulse(amp, n, sigma);
            [outy, tmp] = self.derivGaussSquarePulse(amp*delta, n, sigma);
        end
		...
		%	pat = flipud(conv( flipud(pat), ones(1+padding, 1), 'same' ));
			pat = flipud(conv( flipud(pat), ones(1+padding, 1)));		
            
			% buffer to the right
		%	pat = conv( pat, ones(1+padding, 1), 'same');
			pat = conv( pat, ones(1+padding, 1));
