function [ ] = AWG_PulseTuningPulseGenerator3( AWGHandle, pulseamp, pulsequad, dragamp, num_gate)

%pulseamp is [X90p X180p] 
...
if pulsequad == 0
    ...
    waveform1 = [zeros(1,t_wait-(num_gate+1)*length(gausspulse)-num_gate*t_secpulsewait), pulseamp(1)*amp*gausspulse];
    waveform3 = [zeros(1,t_wait-(num_gate+1)*length(gausspulse)-num_gate*t_secpulsewait), pulseamp(1)*amp*dragpulse];
    if num_gate ~= 0
    ...
    waveform1 = [zeros(1,t_wait-(num_gate+1)*length(gausspulse)-num_gate*t_secpulsewait), pulseamp(1)*amp*dragpulse];
    waveform3 = [zeros(1,t_wait-(num_gate+1)*length(gausspulse)-num_gate*t_secpulsewait), pulseamp(1)*amp*gausspulse];
...
marker1 = zeros(t_wait-t_markbuff-(num_gate+1)*length(gausspulse)-(num_gate)*t_secpulsewait,1);
marker1 = [marker1; 10000*ones(t_markbuff,1); 10000*ones((num_gate+1)*length(gausspulse)+(num_gate)*t_secpulsewait,1); 10000*ones(t_markbuff/2-8,1)];
