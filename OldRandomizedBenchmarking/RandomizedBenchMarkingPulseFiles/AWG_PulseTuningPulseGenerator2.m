function [ ] = AWG_PulseTuningPulseGenerator2( AWGHandle, pulseamp, pulsequad, dragamp, num_gate)

%pulseamp is [X90p X90m] 
...
if pulsequad == 0
    ...
    if num_gate ~= 0
        for counter1=1:num_gate
            waveform1 = [waveform1, zeros(1,t_secpulsewait), pulseamp(1)*amp*gausspulse, zeros(1,t_secpulsewait), pulseamp(2)*amp*gausspulse];
            waveform3 = [waveform3, zeros(1,t_secpulsewait), pulseamp(1)*amp*dragpulse, zeros(1,t_secpulsewait), pulseamp(2)*amp*dragpulse];
        end
    end
else
    ...
    if num_gate ~= 0
        for counter1=1:num_gate
            waveform1 = [waveform1, zeros(1,t_secpulsewait), pulseamp(1)*amp*dragpulse, zeros(1,t_secpulsewait),  pulseamp(2)*amp*dragpulse];
            waveform3 = [waveform3, zeros(1,t_secpulsewait), pulseamp(1)*amp*gausspulse, zeros(1,t_secpulsewait), pulseamp(2)*amp*gausspulse];
        end
    end
end
