function LoadAdvSeq(self, waveforms, markers)
% Low level method to individual waveforms in a sequence
...
%    produce one actual marker sample for each 8 waveform samples.
    ...
    device.Output.OutputMode = 3; % 1 = single waveform
                                  % 2 = sequence
                                  % 3 = advanced sequence
%     device.Arbitrary.Scenario.ClearAll();
%     device.Arbitrary.Scenario.PlayMode = 0; % 0 = Single
                                                                    % 1 =  continuous 
%     device.Arbitrary.Sequence.ClearAll();
    ...
    for ch = 1:2
        ...
        Seqhandle = device.Arbitrary.Sequence.CreateAdvanced(wavehandle,...
                                                                                                            1,... LoopCount
                                                                                                            2, ... AdvanceMode (0 = Auto, 1= Continuous, 2 = One rep, 3 = AllReps
                                                                                                            []); ... Marker mask is not being used as we specify each marker waveforms in MATLAB
       device.Arbitrary.Sequence.Handle(num2str(ch), Seqhandle);                                                                                                     
