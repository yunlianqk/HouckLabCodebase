function dataArray = ReadChannels_v2(self, chList, numReads)
...
%     inArray = double(zeros(arraySize, 1));
    ...
    dataArray = zeros(length(chList)*numReads, params.segments, params.samples);
    
    % Fetch data
    for index = 1:length(chList)*numReads
        chListIndex=floor(index,numReads)+1;
        
        [dataArrayReal64, actualRecords, actualPoints, firstValidPoint, ~, ~, ~, ~] ...
            = device.Channels.Item(device.Channels.Name(chList(chListIndex))).MultiRecordMeasurement.FetchMultiRecordWaveformReal64(0, params.averages*params.segments, 0, params.samples, inArray);
        ...
        % remove zero entries
        dataArray(index, :, :) = tempSeqSig(:, fzirstValidPoint(1)+1:actualPoints(1));
