...
% See QLifTime.T1 as a template.
    ...
    methods
        function self = SingleShot()
            % Constructor, initialize all properties
            self.params = measlib.SingleShot.Params();
            self.instr = measlib.SingleShot.Instr();
            self.data = measlib.SingleShot.Data();
        ...
        % Plot data
            [self.data.intdataI, self.data.intdataQ] ...
                = measlib.SingleShot.integrateData(self.data);
            measlib.SingleShot.plotData(self.data);
        end
    end
